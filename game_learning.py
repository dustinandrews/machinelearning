...
class game_learn:
    ...
    def __init__(self, hp: hyperparameters):
        ...
        self.checkpoint = ModelCheckpoint('model-{epoch:03d}.h5',
                             ...
                             save_best_only=True,
                             mode='auto')
        self.early_stop = EarlyStopping(monitor='val_loss', patience=2, mode='auto')
        self.epoch = 0
...
#                layer.trainable = False
    ...
    def create_model_classify(self, compile_model = True):
        ...
        model.add(Lambda(lambda x: x/127.5-1.0, input_shape=self.input_shape ))
        model.add(Dropout(hp.dropout))
        model.add(Conv2D(24, (5, 5), activation='elu', strides=(2, 2), data_format="channels_first",kernel_regularizer=regularizers.l2(0.01),
                activity_regularizer=regularizers.l1(0.01)))
        model.add(Conv2D(36, (5, 5), activation='elu', strides=(2, 2), data_format="channels_first", kernel_regularizer=regularizers.l2(0.01),
                activity_regularizer=regularizers.l1(0.01)))
        model.add(Dropout(hp.dropout))
        model.add(Conv2D(48, (5, 5), activation='elu', strides=(2, 2), data_format="channels_first",kernel_regularizer=regularizers.l2(0.01),
                activity_regularizer=regularizers.l1(0.01)))
        model.add(Conv2D(64, (3, 3), activation='elu', data_format="channels_first", kernel_regularizer=regularizers.l2(0.01),
                activity_regularizer=regularizers.l1(0.01)))
        model.add(Conv2D(64, (3, 3), activation='elu', data_format="channels_first", kernel_regularizer=regularizers.l2(0.01),
                activity_regularizer=regularizers.l1(0.01)))
        model.add(Dropout(hp.dropout))
        ...
        model.add(Dense(self.output_shape[0], activation='sigmoid', kernel_regularizer=regularizers.l2(0.01),
                activity_regularizer=regularizers.l1(0.01)))
    ...
    def train_model(self, model = None):
        ...
        history = self.model.fit(                
                x_train, 
                y_train,
                callbacks=[self.checkpoint, self.early_stop, self.tbCallBack],
...
if __name__ == '__main__':
    ...
    if len(model_saves) == 0:
        ...
        #tl.trained = models.load_model(file_name)
#    tl.model = tl.create_model_classify_from_autoencoder('saved_weights.h5')
#        
...
#%%
    ...
    plt.show()
    print(p[0][0],p[0][1],p[0][3],p[0][4])
    print(y[0][0],y[0][1],y[0][3],y[0][4])
